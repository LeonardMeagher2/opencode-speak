...
const PRE_ROLL_FRAMES = 30; // ~1 second at 16kHz

let soxProcess: ChildProcess | null = null;
let ringBuf: Buffer[] = [];
let speechBuf: Buffer[] = [];
let inSpeech = false;
let silenceStart = 0;
...
export function startCapture(): void {
  ...
  soxProcess.stdout?.on("data", (chunk: Buffer) => {
    try {
      ...
      // Always accumulate into rolling pre-buffer
      ringBuf.push(frame);
      if (ringBuf.length > PRE_ROLL_FRAMES) ringBuf.shift();
      ...
      if (prob > VAD_SPEECH) {
        if (!inSpeech) {
          inSpeech = true;
          // Copy pre-roll into speech buffer so we don't clip the start
          speechBuf.push(...ringBuf);
        } else {
        ...
        if (silenceStart === 0) {
          ...
          const collected = Buffer.concat(speechBuf);
          ringBuf = [];
          speechBuf = [];
          inSpeech = false;
          silenceStart = 0;
